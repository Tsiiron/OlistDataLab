...
def plot_bad_reviews_by_seller_state(
    df: pd.DataFrame,
    min_orders: int = 200,
    top_n: int = 15
):
    data = df[['seller_state', 'review_score']].dropna()
    ...
    plt.bar(stats['seller_state'], stats['bad_review_share'])
    plt.ylabel('Доля плохих отзывов (1–2)')
    plt.xlabel('Штат продавца')
    plt.title('Доля плохих отзывов по штатам продавцов')
    plt.tight_layout()
    plt.show()
